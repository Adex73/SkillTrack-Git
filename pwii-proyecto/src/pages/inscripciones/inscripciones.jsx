...
function Inscripciones() {
    ...
    const cargarInscritos = useCallback(async (id) => {
        try {
            const res = await fetch(`http://localhost:3000/inscripciones/${id}`);
    ...
    const cargarEstudiantes = useCallback(async (texto = "") => {
        ...
        try {
            ...
            const res = await fetch(`http://localhost:3000/estudiantes/buscar?texto=${texto}&universidad=${userSession.universidad}`);
            const data = await res.json();
            setEstudiantes(data);
        } catch (error) {
