...
function Tareas() {
    ...
    const cargarDatos = async (idCurso, user) => {
        try {
            const res = await fetch(`http://localhost:3000/tareas/${idCurso}`);
            const data = await res.json();
            setTareas(data);
            setTareasOriginales(data);

            if (user.rol === "estudiante") {
                const res2 = await fetch(`http://localhost:3000/entregas/${user.id}`);
    ...
    const subirArchivo = async (e, tareaId) => {
        ...
        try {
            const upload = await fetch("http://localhost:3000/upload", { method: "POST", body: formData });
